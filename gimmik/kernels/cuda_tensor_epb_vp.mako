...
/* CHANGE LOG
    ...
    added pipeline with shared accumulator storage
*/ 
...
__device__ void
${funcn}_line_xy(int k, int eg, int el,
...
__global__ void
__launch_bounds__ (${(block_elem+1)*p*p})
${funcn}(int n,
         ${dtype}* __restrict__ b, int ldb,
         ${dtype}* __restrict__ c, int ldc)
{
    ...
    auto role = (threadIdx.z == blockDim.z-1) ? cuda::pipeline_role::producer : cuda::pipeline_role::consumer;
    auto pipe = cuda::make_pipeline(block, &shared_state, role);
    
    int os = threadIdx.x + threadIdx.y*${p};
    int e0  = blockIdx.x*${block_elem};
    ...
    if(role == cuda::pipeline_role::producer)
    {
        pipe.producer_acquire();
            for (int v=0; v<${nvars}; v++)
                cuda::memcpy_async(tile, bs + SSOA_IDX( 0, v) + (os)*${nvars*vec_width},
                                         b  +  SOA_IDX(e0, v) + (os)*ldb, cuda::aligned_size_t<32>(sizeof(${dtype})*${block_elem}), pipe);
    ...
    for (int k=0; k<${p-1}; k++)
    {
        if(role == cuda::pipeline_role::producer)
        {
            pipe.producer_acquire();
                for (int v=0; v<${nvars}; v++)
                    cuda::memcpy_async(tile, bs + SSOA_IDX(  0, v) + (os + (k+1)*${p*p})*${nvars*vec_width},
                                             b  +  SOA_IDX(e0, v) + (os + (k+1)*${p*p})*ldb, cuda::aligned_size_t<32>(sizeof(${dtype})*${block_elem}), pipe);
            pipe.producer_commit();
        }
        ...
        if(role == cuda::pipeline_role::consumer)
        {
            pipe.consumer_wait();
                if(eg < n)
                    ${funcn}_line_xy(k, eg, el, bs, ${nvars*vec_width}, bs + ${nvars*block_elem*p*p*p}, ${nvars*vec_width});
    ...
    if(role == cuda::pipeline_role::consumer)
    {
        pipe.consumer_wait();
            if(eg < n)
            {    
                ${funcn}_line_xy(${p-1}, eg, el, bs, ${nvars*vec_width}, bs + ${nvars*block_elem*p*p*p}, ${nvars*vec_width});
                ${funcn}_line_z(eg, el, bs, ${nvars*vec_width}, bs + ${nvars*block_elem*p*p*p}, ${nvars*vec_width});
            }
        pipe.consumer_release();
    }
    block.sync();
