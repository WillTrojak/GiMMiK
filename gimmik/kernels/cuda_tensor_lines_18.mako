...
#define SHR_NV ${nvars-1-ndims*(ndims-1)}
...
{
    ${dtype} r[${nvars-(ndims-1)*ndims}];
    r[0] = __ldg(g + SOA_IDX(eg, 0) + (i + j*${p} + k*${p*p})*ldg);
    r[1] = __ldg(g + SOA_IDX(eg, 1) + (i + j*${p} + k*${p*p})*ldg);
    r[2] = __ldg(g + SOA_IDX(eg, 2) + (i + j*${p} + k*${p*p})*ldg);
    r[3] = __ldg(g + SOA_IDX(eg, 3) + (i + j*${p} + k*${p*p})*ldg);
    r[4] = __ldg(g + SOA_IDX(eg, 4) + (i + j*${p} + k*${p*p})*ldg);
    r[5] = __ldg(g + SOA_IDX(eg, 8) + (i + j*${p} + k*${p*p})*ldg);
    r[6] = __ldg(g + SOA_IDX(eg,12) + (i + j*${p} + k*${p*p})*ldg);

    r[4] = ${-c['nu']}*r[4] + r[0];
    r[5] = ${-c['nu']}*r[5] + r[0];
    r[6] = ${-c['nu']}*r[6] + r[0];
                     
    for (int v=0; v<${nvars - 1 - ndims*(ndims-1)}; v++)
       ...
       )
{
    ...
    int oss = 0;
    int osa = ${block_elem*(nvars-1 - (ndims-1)*ndims)*p*p*p};
    int el = tk;
